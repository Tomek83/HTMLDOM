...
class simple_html_dom_node
{
    ...
    function remove(&$__n=null)
    {
	$__n=(is_null($__n)) ? $this : $__n;
	if (is_object($__n))
	{
		if (is_array($__n->children))
		{
			foreach($__n->children as $__c)
			{
				if (is_array($__c->children) && !empty($__c->children))
				{
					$this->remove($__c);
				}
				else
...
class simple_html_dom
{
    ...
    function clear()
    {
        foreach ($this->nodes as $n) {if (is_object($n)) {$n->cleanup();} $n = null;}
        if (isset($this->children)) foreach ($this->children as $n) {if (is_object($n)) {$n->cleanup();} $n = null;}
        if (isset($this->parent)) {if (is_object($this->parent)) {$this->parent->cleanup();} unset($this->parent);}
        if (isset($this->root)) {if (is_object($this->root)) {$this->root->cleanup();} unset($this->root);}
        unset($this->doc);
        unset($this->noise);
    }
